...
library EnumerableStringSet {
    ...
    function _values(Set storage set, uint256 start, uint256 limit) private view returns (string[] memory) {
        require(start < set._values.length, "EnumerableStringSet: start out of bounds");
        uint256 end = set._values.length;
        if (start + limit < end) {
            end = start + limit;
        }
    ...
    function values(StringSet storage set, uint256 start, uint256 limit) internal view returns (string[] memory) {
        string[] memory store = _values(set._inner, start, limit);
        string[] memory result;
